  ...
  if (document.getElementById("usernameButton").value == "False") {
    ...
    socket.on("update-ui", (room_row) => {
      ...
      if (fen != "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1") {
        // this stops new game from creating the right board
        board.position(fen);
      }
      if (live) {
    ...
    socket.on("send-draw-request", (user) => {
      ...
      if (user == username) {
        ...
        game_buttons.insertAdjacentHTML(
          "beforeend",
          "<i id = 'draw-reject' class = 'lni lni-cross-circle' style = 'font-size: 1.5em'></i>",
        );
        game_buttons.insertAdjacentHTML(
          "beforeend",
          "<i id = 'draw-accept' class = 'lni lni-checkmark-circle' style = 'font-size: 1.5em'></i>",
        );
    ...
    socket.on("send-resign-request", () => {
      ...
      game_buttons.insertAdjacentHTML(
        "beforeend",
        "<i id = 'resign-reject' class = 'lni lni-cross-circle' style = 'font-size: 1.5em'></i>",
      );
      game_buttons.insertAdjacentHTML(
        "beforeend",
        "<i id = 'resign-accept' class = 'lni lni-checkmark-circle' style = 'font-size: 1.5em'></i>",
      );
    ...
    socket.on("send-rematch-request", (user) => {
      ...
      if (user == username) {
        ...
        game_buttons.insertAdjacentHTML(
          "beforeend",
          "<i id = 'rematch-reject' class = 'lni lni-cross-circle' style = 'font-size: 1.5em'></i>",
        );
        game_buttons.insertAdjacentHTML(
          "beforeend",
          "<i id = 'rematch-accept' class = 'lni lni-checkmark-circle' style = 'font-size: 1.5em'></i>",
        );
    ...
    socket.on("trigger-draw", (draw) => {
      ...
      if (draw) {
        ...
        document.getElementById("gameOverModalSubtitle").innerHTML =
          "by agreement";
        score += 0.5;
        opp_score += 0.5;
        ...
        game_buttons.insertAdjacentHTML(
          "beforeend",
          "<i id = 'rematch' class = 'lni lni-spinner-arrow' style = 'font-size: 1.5em'></i>",
        );
        document
          .getElementById("rematch")
          .addEventListener("click", requestRematch);
    ...
    function requestRematch() {
      ...
      socket.emit(
        "request_rematch",
        document.querySelector("#opp_username").innerHTML,
      );
    }
    ...
    function updateGameLog(moveArray) {
      // takes in an array of moves
      ...
      if (
        moveArray.length == gameLog.children.length &&
        moveArray.length != 1
      ) {
        return;
      }
      ...
      for (let move of moveArray) {
        if (move) {
          ...
          if (gameLog.children.length % 2 == 0) {
            p.innerHTML =
              (Math.floor(gameLog.children.length / 2) + 1).toString() + ". ";
          }
          p.innerHTML += move;
          gameLog.append(p);
        }
    ...
    function onDragStart(source, piece, position, orientation) {
      ...
      if (
        (game.turn() === "w" && piece.search(/^b/) !== -1) || // add extra checks that side matches the piece moving
    ...
    function checkGameOver() {
      if (game.isGameOver()) {
        ...
        if (game.isCheckmate()) {
          if (isTurn()) {
            document.getElementById("gameOverModalTitle").innerHTML =
              "You Lost!";
            document.getElementById("gameOverModalSubtitle").innerHTML =
              "by checkmate";
            opp_score += 1;
